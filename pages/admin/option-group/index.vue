  ...
  <main class="p-6">
    ...
    <div class="mb-4 text-sm text-gray-700">
      ...
      <span v-else-if="modifiedItems.length > 0">
        {{ modifiedItems.length }}개의 옵션그룹이 변경되었습니다.
      </span>
      <span v-else>
        변경된 내용이 없습니다.
      </span>
          ...
          <tr :key="element.id" class="hover:bg-yellow-50 border-t">
            ...
            <td class="p-2" @click="startEditing(index)">
              <input
                ...
                v-model="element.optionGroupName"
                :id="`input-${index}-optionGroupName`"
                :ref="el => setRef(el as HTMLInputElement, `${index}-optionGroupName`)"
                class="w-full px-2 py-1 border rounded"
                @keydown="e => handleKeydown(e, index, 'optionGroupName')"
                @click.stop
              />
              <span
                v-else
...
const editableItems = ref<OptionGroup[]>([])
const originalItems = ref<OptionGroup[]>([])
const editingIndex = ref<number | null>(null)
const inputRefs = ref<Record<string, HTMLInputElement>>({})
const isServerUpdated = ref(false)
const companyId = getCompanyId();
watch(
  () => optionGroupStore.items.length,
  (len) => {
    if (len > 0 && editableItems.value.length === 0) {
      editableItems.value = optionGroupStore.items.map(c => ({ ...c }))
      originalItems.value = optionGroupStore.items.map(c => ({ ...c }))
    }
  },
  { immediate: true }
)
...
function startEditing(index: number) {
  ...
  nextTick(() => {
    const refKey = `${index}-optionGroupName`
...
function updateDisplayOrder(evt: any) {
  ...
  let newLevel = Math.floor((prevLevel + nextLevel) / 2)
  ...
  if (needsResort) {
    editableItems.value.sort((a, b) => a.displayLevel - b.displayLevel)
      .forEach((c, i) => (c.displayLevel = (i + 1) * 100))
  }
}
...
async function saveAll() {
  ...
  const res = await optionGroupStore.saveItems(companyId!,modifiedItems.value)
  if (res.isSuccess) {
    modifiedItems.value.forEach(modified => {
      const index = optionGroupStore.items.findIndex(c => c.id === modified.id)
      if (index !== -1) optionGroupStore.items[index] = { ...modified }
    })
    originalItems.value = optionGroupStore.items.map(c => ({ ...c }))
    editableItems.value = optionGroupStore.items.map(c => ({ ...c }))
    alert('저장 완료!')
...
function goToEdit(id: string) {
  router.push(`/admin/option-group/edit/${id}`)
}
function goToCreate() {
  router.push(`/admin/option-group/create`)
}
</script>
